...
And let's save it as a csv so we can use it later and not have to run the code that goes so long.
```{r}
race_results_cleaning %>% write_csv("../data/race_results_cleaning.csv", append=FALSE)
```
...
@@LOOK At example in CleanIHME to fix this
