...
**TODO** Figure out what those `*` next to some teams mean. How should we deal with those?
...
While verifying the variables, I've left a list of **TODO**s to look at. Let's try to address some of them in the cleaning:

* For `position`: find out the best explanation of what each of the above codes mean with examples.  
* For `p_prelim`: Figure out what the logic for this part was and try to make sure it's correct.
* For `p_prelim`: Rename the column
* For `team`: Figure out what those `*` next to some teams mean. How should we deal with those?
* For `extra`: Remove random character.
